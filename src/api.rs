...
const CACHE_VERSION: &str = "3"; // v3: Episode.thumbnail + description adı (ep önbelleğini yeniler)
...
pub struct Client {
    ...
    pub(crate) pos_cache: std::sync::Mutex<HashMap<(u64, u64, u64), (std::time::Instant, f64)>>,
    pub(crate) session: std::sync::Mutex<crate::auth::Session>,
}
...
/// Dize ya da `{display_name/name}` nesnelerinden oluşan listeleri
/// `Vec<String>` olarak okur (liste ucu nesne döner, detay ucu dize).
fn de_str_list<'de, D>(d: D) -> Result<Option<Vec<String>>, D::Error>
where
    D: serde::Deserializer<'de>,
{
    let v: Option<Vec<serde_json::Value>> = serde::Deserialize::deserialize(d)?;
    Ok(v.map(|arr| {
        arr.into_iter()
...
pub struct Episode {
    ...
    pub season: u64,
    pub name: String,
    /// Bölüm kapağı (related-videos `thumbnail`; yoksa None → başlık posteri).
    #[serde(default)]
...
/// Sunucudaki izleme kaydı: başlık + son izleme zamanı (ms) +
/// sunucunun bildiği son bölüm (kayıttaki `videos[0]`'dan;
/// season/episode 0 ise bilinmiyor).
#[derive(Clone, Debug, Serialize, Deserialize)]
...
/// Başlık künyesi: oyuncu/ekip kişisi (titles/{id} → credits).
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
pub struct Credit {
    #[serde(default)]
    pub id: u64,
    ...
    #[serde(default)]
    pub poster: Option<String>,
    /// "Acting" / "Directing" / ... (Türkçeleştirme görünümde).
    #[serde(default)]
    pub known_for: String,
}

/// Başlık incelemesi (reviews?titleId=).
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
pub struct Review {
    #[serde(default)]
    pub id: u64,
...
/// Son eklenen bölüm (last-episodes).
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
...
/// Sitedeki haber kaydı (/secure/news): başlık + düz metin gövde +
/// kapak görseli (meta.image) + yayın zamanı.
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
pub struct NewsItem {
    #[serde(default)]
    pub id: u64,
...
/// Gregoryen tarihten hafta günü (0=Pazar). Howard Hinnant algoritması
/// (days-from-civil; 1970-01-01 Perşembe referanslı).
pub fn weekday_num(y: i64, m: u32, d: u32) -> u32 {
    ...
    ((days + 4) % 7).rem_euclid(7) as u32
}

pub fn tr_weekday_short(y: i64, m: u32, d: u32) -> &'static str {
    match weekday_num(y, m, d) {
        0 => "Paz",
        1 => "Pzt",
        2 => "Sal",
        3 => "Çar",
...
pub fn tr_month_short(m: u32) -> &'static str {
    match m {
        1 => "Oca",
        2 => "Şub",
        3 => "Mar",
...
/// "2026-09-07T00:00:40.593Z" → "Pzt 7 Eyl" (gün çipi).
pub fn fmt_tr_day(date_iso: &str) -> String {
    match split_iso(date_iso) {
        Some((y, m, d, _, _)) => {
            format!("{} {d} {}", tr_weekday_short(y, m, d), tr_month_short(m))
        }
        None => date_iso.to_string(),
    }
}

/// "2026-06-23T20:15:20.440Z" → "23.06.2026 20:15" (haber tarihi).
pub fn fmt_tr_datetime(iso: &str) -> String {
    match split_iso(iso) {
        Some((y, m, d, h, min)) => format!("{d:02}.{m:02}.{y} {h:02}:{min:02}"),
        None => iso.to_string(),
    }
}

/// UTC saati TSİ'ye çevirip "20:15" döndürür (yayın saati rozeti).
pub fn fmt_tr_time(iso: &str) -> String {
    match split_iso(iso) {
        Some((_, _, _, h, min)) => {
            let h = (h + 3) % 24;
            format!("{h:02}:{min:02}")
...
pub fn check_internet() -> InternetStatus {
    ...
    /// Keşfet tür adları (Türkçe etiket → site slugu).
    pub const DISCOVER_GENRES: &[(&str, &str)] = &[
        ("Dram", "drama"),
        ("Aksiyon", "action"),
        ("Thriller", "thriller"),
        ("Komedi", "comedy"),
    ...
    /// Keşfet sıralamaları (etiket → `order` parametresi; yok = popülerite).
    ...
    /// Site demetiyle birebir; eşleşmesi bozuk iki girdide (canavar/
    /// doğaüstü) anlam doğru olan kullanıldı.
...
impl Client {
    pub fn new() -> Self {
        ...
        Self {
            ...
            pos_cache: std::sync::Mutex::new(HashMap::new()),
            session: std::sync::Mutex::new(crate::auth::Session::load()),
        }
    }
    ...
    fn cache_get(
        ...
        self.cache_fetch(key, ttl, || loader(&self.http))
    }

    /// Oturum çerezleriyle (girişli uçlar için) önbellekli GET.
    /// `build` hazır çerez header'larını alır, isteği kurar.
    fn cache_get_authed(
        &self,
        key: &str,
        ttl: u64,
        build: impl FnOnce(&crate::http::Http, &[(String, String)]) -> Result<serde_json::Value, String>,
    ) -> Result<serde_json::Value, String> {
        let hdrs = self.auth_headers();
        self.cache_fetch(key, ttl, || build(&self.http, &hdrs))
    }

    /// Oturum çerez header'ları (Cookie + X-XSRF-TOKEN). Girişsiz boş.
    fn auth_headers(&self) -> Vec<(String, String)> {
        let Ok(s) = self.session.lock() else {
            return Vec::new();
        };
        let cookie = s.cookie_header();
        if cookie.is_empty() {
            return Vec::new();
        }
        let mut h = vec![
        ...
        if let Some(x) = s.xsrf() {
            h.push(("X-XSRF-TOKEN".to_string(), x));
        }
        h
    }
    ...
    pub fn episodes(&self, t: &Title) -> Result<Vec<Episode>, String> {
        ...
        // season_count bayat olabilir (örn. listede 1, sitede 3 sezon);
        // bilinen aralığı paralel çek, üstü dolu çıktıkça yokla (max 40).
        let mut seasons = t.season_count.unwrap_or(1).max(1) as u64;

        let results: Vec<Result<Vec<Episode>, String>> = std::thread::scope(|scope| {
            let mut handles = Vec::new();
            for s in 1..=seasons {
                handles.push(scope.spawn(move || self.season_eps(t.id, s)));
            }
        ...
        // Kuyruk yoklaması: en üst sezon doluysa bir üstü dene.
        while seasons < 40 && all.iter().any(|e| e.season == seasons) {
            seasons += 1;
            match self.season_eps(t.id, seasons) {
                Ok(eps) if !eps.is_empty() => all.extend(eps),
                _ => break,
            }
        }
    ...
    /// Tek sezonun bölümleri (6 sa önbellekli).
    fn season_eps(&self, title_id: u64, season: u64) -> Result<Vec<Episode>, String> {
        let key = format!("ep:{title_id}:{season}");
        ...
        let mut season_eps: Vec<Episode> = Vec::new();
        if let Some(vids) = d["videos"].as_array() {
            for v in vids {
                ...
                // Gerçek bölüm adı `description`'da ("Yolculuğun Sonu");
                // `name` genelde jenerik ("1. Bölüm" / "2. Sezon 1. Bölüm").
                let desc = v["description"].as_str().map(|s| s.trim()).filter(|s| !s.is_empty());
                let raw_name = v["name"].as_str().map(|s| s.trim()).filter(|s| !s.is_empty());
                let name = desc.or(raw_name).map(|s| s.to_string())
                    .unwrap_or_else(|| format!("Bölüm {ep}"));
                let thumb = v["thumbnail"].as_str()
                    .map(|s| s.trim())
                    .filter(|s| !s.is_empty())
                    .map(|s| s.to_string());
                if let Some(ex) = season_eps.iter_mut().find(|x| x.season == sez && x.episode == ep) {
                    // Aynı bölümün birden çok videosu olabilir (farklı fansub):
                    // boş description/thumbnail'i dolusuyla tamamla.
                    if ex.thumbnail.is_none() {
                        ex.thumbnail = thumb.clone();
                    }
                    if (ex.name.is_empty() || ex.name.starts_with("Bölüm ")) && !name.starts_with("Bölüm ") {
                        ex.name = name.clone();
                    }
                    continue;
                }
    ...
    /// Bölümün ham video listesi (episode-videos-points, 30dk önbellekli).
    /// Aynı önbellek anahtarını `episode_candidates` ile paylaşır.
    pub(crate) fn episode_points(&self, title_id: u64, episode: u64, season: u64) -> serde_json::Value {
        let key = format!("evp:{title_id}:{season}:{episode}");
        self.cache_get(&key, 1800, |http| {
    ...
    /// Sayı ya da sayı-içeren-metin alanını u64'e çevirir
    /// (takvim/haber uçları iki türde de dönebiliyor).
    fn u64_field(v: &serde_json::Value, key: &str) -> u64 {
        v[key].as_u64().unwrap_or_else(|| {
            v[key]
                .as_str()
    ...
    /// Haftalık yayın takvimi (7 gün) → günler + bölümler. 1 saat önbellekli.
    ...
    /// Başlığın tam detayı (titles/{id}): künye, görseller, oylar... 6 sa önbellekli.
    /// Giriş gerekir (çerezsiz 401).
    pub fn title_detail(&self, title_id: u64) -> Result<serde_json::Value, String> {
        if !self.is_logged_in() {
            return Err("giriş gerekli".to_string());
        }
        let key = format!("tdetail:{title_id}");
        self.cache_get_authed(&key, 6 * 3600, |http, hdrs| {
            let mut r = http
                .get(format!("{BASE}/secure/titles/{title_id}"))
                .header("Accept", "application/json");
            for (k, v) in hdrs {
                r = r.header(k, v);
            }
    ...
    /// Oyuncu & ekip künyesi (Oyunculuk önce, en fazla 12). Girişsiz boş döner.
    pub fn title_credits(&self, title_id: u64) -> Vec<Credit> {
        let d = match self.title_detail(title_id) {
            Ok(d) => d,
            Err(_) => return Vec::new(),
        };
        let mut out = Vec::new();
        if let Some(arr) = d["title"]["credits"].as_array() {
            for c in arr {
                out.push(Credit {
                    id: Self::u64_field(c, "id"),
                    name: Self::str_field(c, "name"),
                    poster: c["poster"]
                        .as_str()
                        .map(|s| s.trim().to_string())
                        .filter(|s| !s.is_empty()),
                    known_for: Self::str_field(c, "known_for"),
                });
            }
        }
        out.sort_by_key(|c| if c.known_for.eq_ignore_ascii_case("acting") { 0u8 } else { 1u8 });
        out.truncate(12);
        out
    }

    /// Benzer başlıklar (titles/{id}/related?limit=5 — siteyle aynı).
    /// Uç limit>5'te detay döndüğü için 5 sabit. Girişsiz boş döner.
    pub fn related_titles(&self, title_id: u64) -> Vec<Title> {
        if !self.is_logged_in() {
            return Vec::new();
        }
        let key = format!("related:{title_id}");
        let d = match self.cache_get_authed(&key, 6 * 3600, |http, hdrs| {
            let mut r = http
                .get(format!("{BASE}/secure/titles/{title_id}/related"))
                .header("Accept", "application/json")
                .query(&[("limit", "5")]);
            for (k, v) in hdrs {
                r = r.header(k, v);
        ...
        let mut out = Vec::new();
        if let Some(arr) = d["titles"].as_array() {
            for t in arr {
                if let Some(title) = Title::from_value(t) {
    ...
    /// Başlık incelemeleri (1-indexli sayfa) → (öğeler, toplam, son_sayfa).
    /// 1 saat önbellekli. Giriş gerekir.
    pub fn reviews(&self, title_id: u64, page: u32) -> Result<(Vec<Review>, usize, u32), String> {
        if !self.is_logged_in() {
            return Err("giriş gerekli".to_string());
        }
        let page = page.max(1);
        let key = format!("reviews:{title_id}:{page}");
        let d = self.cache_get_authed(&key, 3600, |http, hdrs| {
            let mut r = http
                .get(format!("{BASE}/secure/reviews"))
                .header("Accept", "application/json")
                .query(&[
                    ("titleId", &title_id.to_string()),
                    ("page", &page.to_string()),
        ...
        if let Some(arr) = p["data"].as_array() {
            for r in arr {
                out.push(Review {
                    id: Self::u64_field(r, "id"),
                    body: Self::str_field(r, "body"),
                    score: r["score"].as_f64().unwrap_or_else(|| {
                        Self::u64_field(r, "score") as f64
                    }),
                    kind: Self::str_field(r, "type"),
                    created_at: Self::str_field(r, "created_at"),
                    username: Self::str_field(&r["user"], "username"),
    ...
    /// Herkese açık (girişsiz de çalışır). 15 dk önbellekli.
    ...
    /// Sayfa 1-indexli. 1 saat önbellekli.
    pub fn discover(&self, f: &DiscoverFilter) -> Result<(Vec<Title>, usize, u32), String> {
        ...
        if let Some(arr) = arr {
            for v in arr {
                if let Ok(t) = serde_json::from_value::<Title>(v.clone()) {
                    out.push(t);
                }
            }
        }
    ...
    pub fn add_history(&self, t: &Title, e: &Episode) {
        if !self.load_settings().local_history_enabled {
            return;
        }
        let mut st = self.load_state();
...
pub fn parse_tau_embed(url: &str) -> Option<(String, String)> {
    ...
    let after_embed = url.split("/embed/").nth(1)?;
    let (embed_id, vid) = after_embed.split_once("?vid=").unwrap_or((after_embed, ""));
    let embed_id = embed_id.split('/').next()?.to_string();
    let vid = vid.split('&').next().unwrap_or(vid).to_string();
    Some((embed_id, vid))
}
...
mod tests {
    ...
    #[test]
    fn tr_date_helpers() {
        // 2026-09-07 Pazartesi (sitedeki takvim günü).
        assert_eq!(weekday_num(2026, 9, 7), 1);
        assert_eq!(tr_weekday_short(2026, 9, 7), "Pzt");
        assert_eq!(tr_weekday_short(2026, 9, 13), "Paz");
        assert_eq!(fmt_tr_day("2026-09-07T00:00:40.593Z"), "Pzt 7 Eyl");
        assert_eq!(fmt_tr_datetime("2026-06-23T20:15:20.440Z"), "23.06.2026 20:15");
        // UTC 17:00 → TSİ 20:00.
        assert_eq!(fmt_tr_time("2026-09-07T17:00:00.000Z"), "20:00");
        assert!(split_iso("bozuk").is_none());
    }

    #[test]
    fn u64_field_accepts_num_and_str() {
        let v = serde_json::json!({"a": 5, "b": "12", "c": "x"});
        assert_eq!(Client::u64_field(&v, "a"), 5);
        assert_eq!(Client::u64_field(&v, "b"), 12);
        assert_eq!(Client::u64_field(&v, "c"), 0);
        assert_eq!(Client::u64_field(&v, "yok"), 0);
    }

    #[test]
    fn credit_sort_acting_first() {
        let mut v = vec![
            Credit { id: 1, name: "Yönetmen".into(), poster: None, known_for: "Directing".into() },
            Credit { id: 2, name: "Oyuncu".into(), poster: None, known_for: "Acting".into() },
        ];
        v.sort_by_key(|c| if c.known_for.eq_ignore_ascii_case("acting") { 0u8 } else { 1u8 });
        assert_eq!(v[0].name, "Oyuncu");
    }

    #[test]
    fn related_item_parses_via_title() {
        let t = serde_json::json!({
            "id": 7817, "name": "Senko-san", "title_type": "anime",
            "poster": "https://image.tmdb.org/t/p/original/x.jpg",
            "mal_vote_average": 8.7, "season_count": 1
        });
    ...
    #[test]
    fn review_score_num_or_missing() {
        let r = serde_json::json!({"id": 5, "score": 8, "user": {"username": "Ali"}});
        let score = r["score"].as_f64().unwrap_or_else(|| Client::u64_field(&r, "score") as f64);
        assert_eq!(score, 8.0);
        assert_eq!(Client::str_field(&r["user"], "username"), "Ali");
    }

    #[test]
    fn news_item_parses_site_shape() {
        let n = serde_json::json!({
            "id": 401,
            "title": "Örnek Haber",
            "body": "Gövde metni",
            "created_at": "2026-06-23T20:15:20.440Z",
            "meta": {"image": "https://cdn.mangacix.net/x.png", "backdrop": "https://cdn.mangacix.net/y.png"}
        });
        let item = NewsItem {
            id: Client::u64_field(&n, "id"),
            title: Client::str_field(&n, "title"),
            body: Client::str_field(&n, "body"),
            created_at: Client::str_field(&n, "created_at"),
            image: n["meta"]["image"].as_str().map(|s| s.to_string()),
            backdrop: n["meta"]["backdrop"].as_str().map(|s| s.to_string()),
        };
        assert_eq!(item.id, 401);
        assert_eq!(item.title, "Örnek Haber");
        assert!(item.image.unwrap().ends_with("x.png"));
    }
    ...
    fn mark_title_unwatched_clears_all_traces() {
        ...
        let eps = vec![
            Episode { episode: 1, season: 1, name: "B1".into(), thumbnail: None },
            Episode { episode: 2, season: 1, name: "B2".into(), thumbnail: None },
        ];
    ...
    fn apply_watched_list_fills_progress_count() {
        ...
        let eps = vec![
            Episode { episode: 1, season: 1, name: "B1".into(), thumbnail: None },
            Episode { episode: 2, season: 1, name: "B2".into(), thumbnail: None },
            Episode { episode: 3, season: 1, name: "B3".into(), thumbnail: None },
        ];
        c.apply_watched_list(tid, &eps);
